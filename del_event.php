...
if(!isset($_SESSION['userEmail'])){ //check kung nakalog in
...
if (isset($_POST["delete-sched"])){
    $event_id = $_POST['event_id'] ?? '';

    $password=$_POST['userPassword'];
    //$sched_id=$_POST['sched_id'];
 
  ///////////////// check kung match yung pass sa email
  $query = "SELECT userPassword FROM users WHERE userEmail='" . $_SESSION['userEmail'] . "'";
  $result = mysqli_query($conn, $query); //PANG EXECUTE NG QUERY
  
if ($result && $row=mysqli_fetch_assoc($result)){
    if($password==$row['userPassword']){
        ////check sched
        $evSQL=mysqli_query($conn, "SELECT * from create_event where event_id='$event_id'");

            if(mysqli_num_rows($evSQL)>0){
                ...
                if(mysqli_query($conn,$sql)){
                   ...
                   echo "<script>
                        alert('Failed to delete event.');</script>";
                }
      ...
      exit(); // Prevent further execution after alert
  }
}
}
?>
            ...
            <section class="mx-5">
              <div class="row my-5">
                <div class="col-sm-12">
                  <div class="card">
                    <div class="card-body">
                      <h3 class="card-title fw-semibold text-center mb-5">Delete Schedule</h3>
                      <div class="row mx-5">
                        ...
                        <div class="col-12 pb-4">
                          <center><label for="course-description" class="form-label">Enter Password to Delete</label>
                          <input type="password" class="form-control" id="course-description" placeholder="Enter Password" name="userPassword">
</div>
                      </div>
                      ...
                      <button type="submit" class="btn float-end" style="background-color: #3c6e7a; color: white;" name="delete-sched">Save edit</button>
                      </div>
                    </form> 
                  </div>
                </div>
