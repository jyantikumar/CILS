...
$ann_sql = "SELECT * FROM announcement_tbl 
            ...
            ORDER BY ann_id DESC";$ann_result=$conn->query($ann_sql);

$ev_sql = "SELECT DISTINCT event_id, eStart_time, eEnd_time, event_name, status, eStart_date, eEnd_date, repeat_sched
        FROM create_event
        WHERE (
            (repeat_sched IS NOT NULL AND FIND_IN_SET('$current_day', repeat_sched) > 0)
            OR (repeat_sched IS NULL OR repeat_sched = '')
        )
        AND eStart_date <= '$current_date'
        AND eEnd_date >= '$current_date' order by eStart_time asc";

$ev_result=$conn->query($ev_sql);
if (isset($_SESSION['userEmail'])) {
    $userEmail = $_SESSION['userEmail'];

    $getUserSql = "SELECT first_name, last_name FROM users WHERE userEmail = '$userEmail'";
    ...
    if ($getUserResult && mysqli_num_rows($getUserResult) > 0) {
        ...
        $faculty_name = $userRow['first_name'] . ' ' . $userRow['last_name'];
    }
}
else{
...
<div class="row mb-4">
  <div class="col d-flex justify-content-end" style="margin-top:5px;">
    <a href="index.php" class="btn btn-warning">View Classes</a>
  </div>
                ...
                <table class="table">
                  <thead class="table-info">
                      <tr>
                        ...
                        <th scope="col" style="background-color: #3c6e7a; color: white;">Event Name</th>
                        <th scope="col" style="background-color: #3c6e7a; color: white;">Status</th>
                      </tr>
                    </thead>
                    <tbody>
                    <?php if ($ev_result->num_rows > 0): ?>
                                <?php while ($row = $ev_result->fetch_assoc()): ?>
                                    <tr>
                                    <td><b><?php echo htmlspecialchars(date("h:i A", strtotime($row['eStart_time'])) . " - " . date("h:i A", strtotime($row['eEnd_time']))); ?></b></td>
                                    <td><?php echo htmlspecialchars($row['eStart_date']); ?></td>
                                    <td><?php echo htmlspecialchars($row['event_name']);?></td>

                                    <td>                                       <?php 
                                      $schedDate = $row['eStart_date'];
                                      $schedStart = $row['eStart_time'];
                                      $schedEnd = $row['eEnd_time'];
                                ...
                                <?php else: ?>
                                <tr>
                                  <td colspan="6">No events in the laboratory today.</td>
                                </tr>
                                <?php endif; ?>     
                    </tbody>
              </div>
