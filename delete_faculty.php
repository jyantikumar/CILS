...
if ($result && $row=mysqli_fetch_assoc($result)){
    if($password==$row['userPassword']){
        ////check 
        $facultySQL=mysqli_query($conn, "SELECT * from faculty_tbl where faculty_id='$faculty_id'");

            if(mysqli_num_rows($facultySQL)>0){
                $sql="update faculty_tbl set faculty_status='ARCHIVED' where faculty_id='$faculty_id'";
                if(mysqli_query($conn,$sql)){
                    header("location:faculty-management.php");
                    exit();     

                }else {
                  echo "<script>alert('Failed to archive faculty.'); window.location.href='faculty-management.php';</script>";
                  exit();
              }
            ...
            <section class="mx-5">
              <div class="row my-5">
                <div class="col-sm-12">
                  <div class="card">
                    <div class="card-body">
                      <h3 class="card-title fw-semibold text-center mb-5">Archive Faculty</h3>
                      <div class="row mx-5">
                        ...
                        <div class="col-12 pb-4">
                          <center><label for="course-description" class="form-label">Enter Password to Archive</label>
                      ...
                      <button type="submit" class="btn float-end" style="background-color: #3c6e7a; color: white;" name="delete-faculty">Archive</button>
                      </div>
                    </form>
                  </div>
                </div>
