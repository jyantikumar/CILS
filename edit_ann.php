...
// Check if ann_id is set in the URL and fetch announcement data
if (isset($_GET['ann_id'])) {
    $ann_id = $_GET['ann_id'];

    // Fetch the announcement details
    $sql = "SELECT * FROM announcement_tbl WHERE ann_id = '$ann_id'";
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        $ann_start = $announcement['ann_start'];
        $ann_end = $announcement['ann_end'];
    } else {
...
if (isset($_POST['ann_id'])) {
    ...
    if (strtotime($ann_start) < strtotime($ann_end)) {
        echo "<script>alert('Announcement expiry date cannot be before start date');</script>";

    }
  
    // Update the announcement
    $sql = "UPDATE announcement_tbl SET ann_title='$ann_title', ann_content='$ann_content', ann_start='$ann_start', ann_end='$ann_end' WHERE ann_id='$ann_id'";

    if (mysqli_query($conn, $sql)) {
        header("Location: view_ann.php"); // Redirect to view announcements page after update
        exit();
    } else {
...
<li class="sidebar-item">
        ...
        <div class="main">
            ...
            <div class="container" style="max-width: 1920px;">
                <h3 class="text-center my-5">Edit Announcement</h3>
                <div class="row mb-5">
                <form method="POST" action="">
                <input type="hidden" name="ann_id" value="<?php echo isset($ann_id) ? $ann_id : ''; ?>">

    <div class="row">
        <div class="col-6 pb-4">
        <label for="ann_start" class="form-label">Announcement Start Date</label>
        <input type="date" class="form-control" name="ann_start" value="<?php echo isset($ann_start) ? $ann_start : ''; ?>" required>
        </div>
        <div class="col-6 pb-4">
        <label for="ann_end" class="form-label">Announcement Expiry Date</label>
        <input type="date" class="form-control" name="ann_end" value="<?php echo isset($ann_end) ? $ann_end : ''; ?>" required>
        </div>
    </div>

    <div class="row">
        <div class="col-12 pb-2">
        <input type="text" class="form-control" placeholder="Announcement Title" name="ann_title" value="<?php echo isset($ann_title) ? $ann_title : ''; ?>" required><br>
        <textarea class="ann-content form-control" placeholder="Input Content" name="ann_content" rows="5" cols="10" required><?php echo isset($ann_content) ? $ann_content : ''; ?></textarea>
        </div>
    </div>
