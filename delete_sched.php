...
if (isset($_POST["delete-sched"])){
    ...
    if ($result && $row = mysqli_fetch_assoc($result)){
        ...
        if ($password == $row['userPassword']){
            ...
            $schedSQL = mysqli_query($conn, "SELECT * FROM create_sched WHERE sched_id='$sched_id'");

            if(mysqli_num_rows($schedSQL) > 0){
                // Delete the schedule
                $sql = "DELETE FROM create_sched WHERE sched_id='$sched_id'";
